...
# $Header: fatools/opensource/jauditFixScripts/FinPreMerge/bin/fin_premerge.ksh /st_fatools_11.1/23 2015/10/12 17:42:25 haripate Exp $
#
# fin_premerge.ksh
#
...
VersionCmd=$cmd_dir/GetVer
VersionFile=$cmd_dir/Version.info
BugVersionFile=$cmd_dir/BugVersion.info

junitdb_file=~/.JUnit.db
bugdb_file=~/.BugUser.info
bugUid=fintools

idsToCopyMail="fusion-premerge_ar@oracle.com"
...
FTPMAXLEN=460
FTPMaxParams=16
BugFTPServer=sftp://bugsftp.us.oracle.com
FTPUser=fusion-premerge_ar@oracle.com
FTPPass="NoPass"
ERR=2048
ABORT=-1
export debug=0
...
BreakOnError=1
PerformSeedValidation=1
PerformFreValidation=1
PerformPseudoTranslation=1
timeOut=86400 #One day!

showBanner()
{
	echo
	echo "		Fusion Pre-merge Validation Tool"
	echo "		--------------------------------"
	echo
...
showUsageAndExit()
{
	...
	echo "      Multiple projects can be passed as a comma separated list or by repeating -j."
        ...
        echo "##############################################################"
        echo
        echo "  Read more about the tool at https://kix.oraclecorp.com/KIX/display.php?labelId=1995&articleId=276161"
        echo "  Issues, ERs, Suggestions? Please follow the FAQ link at the bottom of the above KIX page."
        echo
...
SendMail()
{
	...
	if [[ $mailSent -eq 0 ]];then
		...
		echo >>$outfile				
		if [[ $1 -eq $ABORT ]];then
			cat $outfile "$destdir"/$txnDescFile 2>/dev/null|mailx -s"[Fusion-Pre-mergereq] - Processing $status for $txn in $ADE_VIEW_NAME" $email -c "$idsToCopyMail"
		else
			cat $outfile "$destdir"/"$txn"_*.out 2>/dev/null|mailx -s"[Fusion-Pre-mergereq] - Processing $status for $txn in $ADE_VIEW_NAME" $email -c "$idsToCopyMail" 
		fi
		mailSent=1
	fi
	mail_end=$(date +%s)
...
setBugContext()
{
	...
	newCon='(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=amogridxp01-scan.us.oracle.com)(PORT = 1523))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME = bugap_adx.us.oracle.com)))'
...
if [[ -z $ADE_VIEW_ROOT ]];then
	echo "*************************************************************"
	echo "This command must be executed from within a view. Cannot proceed."
	echo "*************************************************************"
	exit 99
fi
...
if [[ $runARUValid -eq 1 ]];then
     if [[ $family -eq "fin" || $family -eq "prj" ]];then
...
echo "Running Bad SQL Headers scan."
...
echo "Running Unused TaskFlows scan."
...
echo "Running ADFdi RowLevelBingding Scans."
...
echo "Running ADFdi VersionCheck Scan."
...
PidList="$pidBld $pidJut $pidAdt $pidGsc $pidStr $pidCntrl $pidHdr $pidCptn $pidPasv $pidInvRef $pidDepTrig $pidSqlhdr $pidRct $pidFdr $pidPtch $pidSelectCnt $pidSdoInterface $pidUnusedTF $pidVCScan $pidVCPerfScan $pidUtplsql $pidRSIScan $pidBIScan $pidUISearchScan $pidSysdateScan $pidEnumScan $pidAutoExecuteVCScan $pidVcMethodAction $pidAdfdi $pidAdfrowlevelbinding $pidSeed $pidPlsqlAuto $pidContentDelivery $pidAdfdiVersionCheck $pidBindVarCheck $pidSeedEndDates $pidSessionTimeout $pidTranslatorNotes $pidFre $pidPseudo $pidHardCodeStr $pidUnusedAutoSubmit" 
...
if [[ $retCntrl -eq -2 ]];then
        echo "***Terminated - Central files check."|tee -a $outfile
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - No central files found in the transaction." |tee -a $outfile
	filesForBugFtp="$filesForBugFtp"' '"$txn"_centralfilechecker.out
...
if [[ $retStr -eq -2 ]];then
        echo "***Terminated - Strings check."|tee -a $outfile
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - Strings check." |tee -a $outfile
	filesForBugFtp="$filesForBugFtp"' '"$txn"_stringchecker.out
elif [[ $retStr -eq 1 ]];then
        echo "***Failure - Strings check." |tee -a $outfile
	errCode=`expr $errCode + 32`
	isErr=1
	filesForBugBody="$filesForBugBody"' '"$destdir"/"$txn"_stringchecker.out
elif [[ $retStr -eq 2 ]];then
        echo "*Warning - Strings check did not complete successfully. Please check manually."|tee -a $outfile
	errCode=`expr $errCode + 32`
elif [[ $retStr -eq 3 ]];then
	echo "Skipped - Strings check."|tee -a $outfile
	filesForBugFtp="$filesForBugFtp"' '"$txn"_stringchecker.out
else
	echo "*Warning - Strings check returned unexpected error - $retStr. Please check the output manually."|tee -a $outfile
	errCode=`expr $errCode + 32`
	filesForBugFtp="$filesForBugFtp"' '"$txn"_stringchecker.out
fi
...
if [[ $retInvRef -eq -2 ]];then
	...
	echo "Success - Invalid Jar References scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_invalidref.out
...
if [[ $retDepTrig -eq -2 ]];then
	...
	echo "Success - Deprecated Triggers scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_deprecatedtriggers.out
...
if [[ $retRct -eq -2 ]];then
	...
	echo "Success - RowCount Threshold scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_rct.out
...
if [[ $retSdoInterface -eq -2 ]];then
	...
	echo "Success - SDO Interface scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_sdoInterface.out
...
if [[ $retUnusedTF -eq -2 ]];then
	...
	echo "Success - Unused TaskFlow scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_unusedTF.out
...
if [[ $retVcScan -eq -2 ]];then
	...
	echo "Success - ViewCriteria scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_vc.out
...
if [[ $retVcPerfScan -eq -2 ]];then
	...
	echo "Success - ViewCriteria  performance scans (BadViewCriteria/NoViewCriteria)." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_vcPerf.out
...
if [[ $retRSIScan -eq -2 ]];then
	...
	echo "Success - RowSetIterator  scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_rsi.out
...
if [[ $retBIScan -eq -2 ]];then
	...
	echo "Success - BIExtenderAutomation  scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_bi.out
...
if [[ $retVcMethodAction -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - Method Action with VC." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_methodActionVc.out
...
if [[ $retAutoExecuteVCScan -eq -2 ]];then
	...
	echo "Success - AutoExecuteVC  scan." 
	filesForBugFtp="$filesForBugFtp"' '"$txn"_autoexecute.out
...
if [[ $retAdfrowbinding -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - Adfdi RowlevelBinding Scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_adfdiRowLevelBindingCheck.out
...
if [[ $retPlsqlAuto -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - PLSql Autonomous scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_plsqlAutonomous.out
...
if [[ $retContentDelivery -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - ContentDelivery scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_contentDelivery.out
...
if [[ $retUnusedAutoSubmit -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - UnusedAutoSubmit & NonExistentPartialTrigger scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_unusedAutoSubmit.out
...
if [[ $retAdfdiVersionCheck -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - ADFdiVersionCheck scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_adfdiVersionCheck.out
...
if [[ $retBindVarCheck -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - BindVariable Kind Mismatch scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_bindVarKindMismatch.out
...
if [[ $retSeedEndDates -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - SeedEndDates scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_seedEndDate.out
...
if [[ $retHardCodeStr -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - Hard Coded Strings Check." 
        filesForBugFtp="$filesForBugFtp"' 'hardcodeStrChk.trace
        filesForBugFtp="$filesForBugFtp"' 'rptHardStr.csv
...
if [[ $retSessionTimeout -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - SessionTimeout scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_sessionTimeout.out
...
if [[ $retTranslatorNotes -eq -2 ]];then
	...
	if [[ -n  "$TIME_LOG" ]]; then
        ...
        echo "Success - AvoidLoadingTranslatorNotes scan." 
        filesForBugFtp="$filesForBugFtp"' '"$txn"_avoidTranslator.out
...
if [[ $retBld -eq 1 || $retBld -eq -2 ]];then
	...
	PidList="$pidFullBld $pidJut $pidVOTuningV3"
	$cmd_showprogress "Please wait while the validations are progressing in the background " "$PidList" &
	pidPrgs=$!
