...
`echo "Started processing at $start_time" > $log_dir/${1}.log`
...
if [  "$length" -gt 1 ]  ;then
	...
	do 
		if [ "$var" =  "no_upload" ] ; then  
			...
			index=$index + 1
			echo "Found transaction to fetch: $var" | tee -a $log_dir/${1}.log
		fi
	done
fi
...
task_start_time=$(date +%s)
echo -n "Grabbing transactions..." | tee -a $log_dir/${1}.log

for x in "${transNames[@]}"
do
	`ade useview $viewname -silent -exec "ade grabtrans $x &> $log_dir/grabtrans_${x}.log" &> $log_dir/useview.log`
	echo "Grabbed transaction $x"
done
...
if [ "$run_build" = "Y" ]; then
	...
	if [ "$success" = "SUCCESS" ]; then  # the report shows a successful compile
		echo "Generating ears..." | tee -a $log_dir/${1}.log
		`ade useview $viewname -silent -exec "ant ear -f hed/build.xml &> $log_dir/hed_ear.log" &> $log_dir/ear.log`		
	else
		echo "Compile failed. Not generateing ears" | tee -a $log_dir/${1}.log
	fi
...
if [ "$run_test" = "Y" ]; then
	...
	if [ "$run_HER_test" = "Y" ]; then
		...
		if [ "$junit_full_suite" = "N" ]; then
			...
			echo -n "Running HER Junits..." | tee -a $log_dir/${1}.log
			`ade useview $viewname -silent -exec "ade pwv" -exec "ant -Dtest.lrg=true -Demma.enabled=$emma -Ddb.host=$junit_db_mc -Ddb.port=$junit_port -Ddb.sid=$junit_sid -f hed/build-recordsManagement.xml test test-report &> $log_dir/HER_test-report.log"`
			task_end_time=$(date +%s)
			getElapsedTime	
			echo "Done. Task completed in $elapsed_time_formatted" | tee -a $log_dir/${1}.log
		fi
...
if [[ -n $email ]];then
	...
	if [ -f $log_dir/${1}_build-report.html ]; then
		attachments=" -a $log_dir/${1}_build-report.html"
	fi
